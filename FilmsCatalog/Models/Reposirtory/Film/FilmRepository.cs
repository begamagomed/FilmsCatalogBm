...
namespace FilmsCatalog.Models
{
    public class FilmRepository
    {
        ...
        public IEnumerable<Film> GetFilms(int offset = 0, int count = int.MaxValue)
        {
            return this.context.Films.AsNoTracking().OrderBy(f => f.Id).Skip(offset).Take(count).AsEnumerable();
        }
    }
}
